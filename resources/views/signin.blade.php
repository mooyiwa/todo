@extends('layouts.signup')

@section('signin')


{!! Form::open(array('action' => 'pagesController@log_in'))  !!}
<ul class="sign">
<li><p class='msg' style="position:relative; left:-6px;">{{ @$msg }}</p></li>
<li>{!! Form::text('user',null,['class' => 'ent', 'placeholder' => 'username']) !!}</li>

<li>{!! Form::password('pass',['class' => 'ent', 'placeholder' => 'password']) !!}</li>


<li>{!! Form::submit('Sign in',['class' => 'button']) !!}</li>
<li><p style="margin-top: 10px">New User? <a href="/site/signup">Sign Up!</a></p></li>
<li><p style="margin-top: 10px"><a href="/site/recover">Forgot password?</a></p></li>
</ul>

{!! Form::close() !!}



@stop


